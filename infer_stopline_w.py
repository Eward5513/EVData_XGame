...
INTERSECTION_JSON_PATH = f"{DATA_DIR}/data/intersection_A.json"  # JSON with lane_divider
...
def main():
    ...
    with open(f"{OUT_DIR}/stopline_west.json", "w", encoding="utf-8") as f:
        json.dump(stoplines, f, ensure_ascii=False, indent=2)

    # 2) Analysis file (both roads combined for inspection)
    with open(f"{OUT_DIR}/analyse_stopline_w.json", "w", encoding="utf-8") as f:
        json.dump(analyses, f, ensure_ascii=False, indent=2)

    print("[OK] 输出完成：")
    print(f"  - {OUT_DIR}/stopline_west.json")
    print(f"  - {OUT_DIR}/analyse_stopline_w.json")
